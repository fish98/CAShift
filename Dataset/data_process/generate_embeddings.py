    ...
    if strategy == 'average':
        ...
        for i, (template, k) in enumerate(templates):
            for word in template.strip().split():
                if word in words:
                    #words=words.tolist()
                    single_weights.append(tfidf[i][words.tolist().index(word)])
                else:
...
if __name__ == '__main__':
    ...
    dataset = sys.argv[2]
    strategy = sys.argv[3]
    print(f'Generating embeddings for {dataset} in {dirname} using {strategy}...')

    # Prepare template_full as all syscalls
    template_full = []

    # Read all csv in directory
    for filename in os.listdir(dirname):
        df = pd.read_csv(os.path.join(dirname, filename))
        syscall_list = df["SysCall"].unique().tolist()
        print(f"{filename} has {len(syscall_list)} syscalls")
        template_full.extend(syscall_list)

    # remove duplicates
    template_full = list(set(template_full))
    print(f"Total syscall number: {len(template_full)}")

    # generate embeddings
    embeddings = generate_embeddings_fasttext(template_full, strategy=strategy)
    # save embeddings
    with open(os.path.join(dirname, f'{dataset}.log_embeddings_{strategy}.json'), 'w') as f:
